...
function finish(state: GameState, playerId: string): void {
  const index = state.deck.findIndex((card) => card.id === 'zombie_unicorn');
  if (index !== -1) {
    const [zombie] = state.deck.splice(index, 1);
    const player = state.players.find((candidate) => candidate.id === playerId);
    if (player) CardMovement.enterStable(state, player, zombie);
  }
...
export const zombieApocalypse: CardEffect = {
  onPlay(state, player) {
    ...
    state.pendingAction = {
      ...
      reason: 'zombie_apocalypse_destroy',
