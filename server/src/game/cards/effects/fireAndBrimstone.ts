...
function finish(state: GameState, playerId: string): void {
  const index = state.deck.findIndex((card) => card.id === 'unicorn_phoenix');
  if (index !== -1) {
    const [phoenix] = state.deck.splice(index, 1);
    const player = state.players.find((candidate) => candidate.id === playerId);
    if (player) CardMovement.enterStable(state, player, phoenix);
  }
...
export const fireAndBrimstone: CardEffect = {
  onPlay(state, player) {
    ...
    state.pendingAction = {
          ...
          reason: 'fire_and_brimstone_destroy',
