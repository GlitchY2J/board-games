...
function finish(state: GameState, playerId: string): void {
  const index = state.deck.findIndex((card) => card.id === 'angel_unicorn');
  if (index !== -1) {
    const [angel] = state.deck.splice(index, 1);
    const player = state.players.find((candidate) => candidate.id === playerId);
    if (player) CardMovement.enterStable(state, player, angel);
  }
...
export const heavenlySmite: CardEffect = {
  onPlay(state, player) {
