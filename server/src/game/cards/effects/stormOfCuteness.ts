...
function finish(state: GameState, playerId: string): void {
  const index = state.deck.findIndex((card) => card.id === 'magical_kittencorn');
  if (index !== -1) {
    const [kittencorn] = state.deck.splice(index, 1);
    const player = state.players.find((candidate) => candidate.id === playerId);
    if (player) CardMovement.enterStable(state, player, kittencorn);
  }
...
export const stormOfCuteness: CardEffect = {
  onPlay(state, player) {
    ...
    state.pendingAction = {
      ...
      reason: 'storm_of_cuteness_destroy',
