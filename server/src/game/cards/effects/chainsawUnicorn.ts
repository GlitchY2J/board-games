...
import { isProtectedByParanormalAffection } from './paranormalAffection.ts';
  ...
  onEnterStable(state, player, card) {
    ...
    const hasOtherUpgrades = state.players.some(
      (p) =>
        ...
        p.upgrades.some(
          (upgrade) =>
            !isProtectedByParanormalAffection(p, upgrade) &&
            !isImmuneToUnicornOrUpgradeDestruction(upgrade.id),
        ),
    );
