...
export function registerActionHandlers(io: Server, socket: Socket): void {
  ...
  function registerDiscardCards(io: Server, socket: Socket): void {
    socket.on(
      'discard-cards',
      ({
        roomCode,
  ...
  function registerSelectPlayer(io: Server, socket: Socket): void {
    socket.on(
      'select-player',
      ({
        roomCode,
  ...
  function registerSelectStableCard(io: Server, socket: Socket): void {
    socket.on(
      'select-stable-card',
  ...
  function registerSelectHandCard(io: Server, socket: Socket): void {
    socket.on(
      'select-hand-card',
      ({ roomCode, cardId }: { roomCode: string; cardId: string }) => {
        const context = getSocketGameContext(socket, roomCode);
  ...
  function registerCancelAction(io: Server, socket: Socket): void {
  ...
  function registerSelectChoice(io: Server, socket: Socket): void {
    socket.on(
      'select-choice',
  ...
  function registerSelectDiscardCard(io: Server, socket: Socket): void {
    socket.on(
      'select-discard-card',
      ({ roomCode, cardId }: { roomCode: string; cardId: string }) => {
        const room = roomManager.getRoom(roomCode);
